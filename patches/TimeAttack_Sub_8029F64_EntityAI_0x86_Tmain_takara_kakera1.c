// @Description TimeAttack_Sub_8029F64_EntityAI_0x86_Tmain_takara_kakera1
// @HookAddress 0x78EA18
// @HookString P
// Made by beco
// Verison 1.0
...
#define Sub_8029F64_EntityAI_0x86_Tmain_takara_kakera1 ((void (*)()) 0x8029F65)
...
void TimeAttack_Sub_8029F64_EntityAI_0x86_Tmain_takara_kakera1() {
    // Custom code
    if (CurrentEnemyData.CurEnemy_CurrentAnimationId == Q_K5_HIP) {
        ...
        ItemNum ++;
        /*
        int a = LapTimes[i]+(LapTimes[i+1]*10)+(LapTimes[i+2]*100)+(LapTimes[i+3]*1000)+(LapTimes[i+4]*10000)+(LapTimes[i+5]*100000);
        int b = FrameDigit1+(FrameDigit2*10)+(SecondDidit1*100)+(SecondDidit2*1000)+(MinuteDidit1*10000)+(MinuteDidit2*100000);
        int c = a - b;
        DeltaTime[0] = c >> 5 & 10;
        DeltaTime[1] = c >> 4 & 10;
        DeltaTime[2] = c >> 3 & 10;
    ...
    // Vanilla code
    Sub_8029F64_EntityAI_0x86_Tmain_takara_kakera1();
}
